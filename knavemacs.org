...
*** Color Theme
#+BEGIN_SRC emacs-lisp
  (if (not(eq system-type 'windows-nt )) (load-theme 'modus-vivendi t))
  (if (eq system-type 'windows-nt) (load-theme 'modus-vivendi t))
#+END_SRC

*** Modeline
#+BEGIN_SRC emacs-lisp
  ;; SETUP: telephone-line
  ;; PURPOSE: provide an aesthetic, configurable mode line for emacs
  ;; DEP: may need to change telephone-line-height based on font settings
  (use-package telephone-line
    ...
    (setq telephone-line-height 24) ; setting manually because of font settings
    )
#+END_SRC
