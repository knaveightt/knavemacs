...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (setq org-todo-keywords
          '((sequence "TASK(k)" "TODO(t)" "PROJ(p)" "NEXT(x)" "FOLLOWUP(f@/!)" "HOLD(h@/!)" "JIRA(j)" "|" "DONE(d!)" "CANCELED(c)")
            (sequence "NOTICE(n)" "|" "RESOLVED(r@)")
            (sequence "NOTE(f)" "|" "NOTED(z)" "TRASH(x)")
            ))
    (setq org-todo-keywoard-faces
          '(("TODO" . "green") ("PROJ" . "red") ("NEXT" . "green") ("FOLLOWUP" . "purple") ("HOLD" . "yellow") ("JIRA" . "green")
    ...
    (setq org-agenda-custom-commands
          '(("d" "Work-Week Dashboard"
             (
              ...
              (todo "TASK" ((org-agenda-overriding-header "Tasks for Today")))
              (tags "PROJECT+TODO=\"NEXT\"|-PROJECT+TODO=\"TODO\""
                         ((org-agenda-overriding-header "Activities to Schedule")
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'scheduled))))
              (todo "FOLLOWUP"
                    ((org-agenda-overriding-header "Need to Follow Up Items")))))
    ...
    (setq org-capture-templates
          '(
            ...
            ("j" "Track New JIRA" entry (file+olp "~/Documents/org/unfiled.org" "Todos")
             "* JIRA [/] %^{Enter Issue Name} :PROJECT:%^{Enter Jira Number w Underscores}:\n** NEXT %^{Enter First Action}\n** TODO %?" :empty-lines 1)
