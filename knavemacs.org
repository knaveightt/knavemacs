...
- dunst (notifications) if on linux
...
** Wrap Up Processes
If I'm running linux, I'm probably using emacs as a daemon.
Based on this, I run a shell command to notify that the emacs configuration has been loaded.
This requires dunst or an equivalent notification system to be installed.
#+BEGIN_SRC emacs-lisp
